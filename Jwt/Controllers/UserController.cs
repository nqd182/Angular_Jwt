...
namespace Jwt.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public UserController(MyDbContext context, IOptions<AppSetting> otionsMonitor) //  IOptions để lấy cấu hình đã đăng ký bằng Configure.
        {
            _context = context;
        ...
        private async Task<TokenModel> GenerateToken(NguoiDung user)        
        {
            // code tạo token
        ...
        public async Task<IActionResult> RenewToken(TokenModel model)
        {
            ...
            try
            {
                ...
                if (validatedToken is JwtSecurityToken jwtSecurityToken)
                {
                    ...
                    await _context.SaveChangesAsync();
                    // cấp lại token
                    var user = _context.NguoiDungs.FirstOrDefault(x => x.Id == storedRefreshToken.UserId);
                    var token = await GenerateToken(user);
                    return Ok(new ApiResponse
                    {
                        Success = true,
